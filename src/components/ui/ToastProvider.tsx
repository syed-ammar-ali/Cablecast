...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ showToast, toast, confirm }}>
      ...
      {/* ── Toast Container (Responsive positioning: top on mobile and desktop to clear handheld remote) ── */}
      <div
        aria-live="polite"
        className="pointer-events-none fixed top-[max(0.75rem,env(safe-area-inset-top))] sm:top-5 sm:bottom-auto inset-x-3 sm:inset-x-auto sm:right-5 z-[9990] flex flex-col gap-2.5 max-w-sm sm:max-w-md mx-auto sm:mx-0 w-auto"
      >
        {toasts.map((item) => {
            ...
            return (
              <div
                ...
                className={`pointer-events-auto relative overflow-hidden rounded-2xl border border-amber-500/40 bg-neutral-950/95 p-3.5 sm:p-4 shadow-2xl shadow-black/90 backdrop-blur-2xl transition-all duration-200 ${
                  isDismissing
                    ? "animate-out fade-out slide-out-to-top-3 pointer-events-none"
