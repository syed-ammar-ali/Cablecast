...
export async function GET() {
  try {
    ...
    if (!session) {
      return NextResponse.json({ favorites: [], owned: [], rented: [], collection: [] });
    }
    ...
    async function resolveMetadata(
      ...
      try {
        ...
        console.warn(`[all] Metadata resolution fallback for ${mediaType}_${mediaId}:`, err);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to load library", favorites: [], owned: [], rented: [], collection: [] },
      { status: 500 },
    );
  }
}
